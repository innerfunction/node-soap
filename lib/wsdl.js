        ...
        */
        ...
        case 'output':
            ...
            if (message.element) {
                ...
                this[child.name] = { description: function() {} };
            }
            children.splice(i--,1);
            break;
        case 'fault':
            this.fault = { description: function() { return 'oh my what to do'; } };
        }
        // INFU edit ----
    }
    this.deleteFixedAttrs();
...
WSDL.prototype.objectToDocumentXML = function(name, params, ns, xmlns) {
    ...
    /** INFU edit ----
     * This change is to correct output of the EmptyResponse type in the NEC WSDL.
     * Only include an 'args' var if 'name' is defined'
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns) {
    ...
    if (Array.isArray(obj)) {
        ...
        for (var i=0, item; item=obj[i]; i++) {
            ...
            // INFU FIX: Apply namespace prefix to array elements.
            parts.push(self.objectToXML(item, name, namespace, xmlns));
            // INFU FIX: -----------------------------------------
        }
        */
        // INFU FIX: Fix array serialization.
        ...
        for (var name in obj) {
            var child = obj[name];
            // INFU FIX: Use xsi:nil for null object refs.
            if( child === null ) {
                parts.push('<'+ns+name+xmlnsAttrib+' xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:nil="true"/>');
            }
            // INFU FIX: --------------------------------
            else {
                parts.push(['<',ns,name,xmlnsAttrib,'>'].join(''));
                // INFU FIX: Apply namespace prefix to child elements.
