...
*/
// New method executor. This version includes proper SOAP fault handling. Method handlers should
// now pass {fault}, {result} args to the callback function. Any exceptions thrown by a method
// handler are treated as faults.
Server.prototype._executeMethod = function(options, callback) {
    options = options || {};
    var self = this,
        method, body,
    ...
    function handleResult( fault, result ) {
        // Replace this function with a noop to prevent it being invoked twice.
        handleResult = function() {};
            
        if( fault ) {
            outputName = fault.name;
        }
        ...
        if( fault ) {
            body = self._fault('soap:Client', fault.name, body );
        }
...
// INFU edit ----
Server.prototype._fault = function( code, name, body ) {
    return '<soap:Fault>'+
               '<faultcode>'+code+'</faultcode>'+
               '<faultstring>'+name+'</faultstring>'+
               '<detail>'+body+'</detail>'+
           '</soap:Fault>';
}
// INFU edit ----
